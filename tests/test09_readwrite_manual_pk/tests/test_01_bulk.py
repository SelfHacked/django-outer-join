...
def test_bulk_create():
    ...
    assert A1._base_manager.filter(key__in=[5, 6]).count() == 2
...
def test_batch_update():
    ...
    assert set(A1._base_manager.values_list('key', flat=True)) == {1, 2, 3, 4}
    ...
    assert A1._base_manager.get(key=1).field1 == 15
    assert A1._base_manager.get(key=2).field1 == 15
    assert A1._base_manager.get(key=3).field1 == 15
    assert A1._base_manager.get(key=4).field1 is None
